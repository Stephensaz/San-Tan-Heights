...
class AgentCardView:
    ...
    body: str | None
    status_label: str | None
    findings: tuple[AgentFindingView, ...]


@dataclass(frozen=True)
class AgentSectionView:
    section_id: str
    title: str
    summary: str | None
    cards: tuple[AgentCardView, ...]
...
class AgentReportView:
    ...
    summary: tuple[str, ...]
    sections: tuple[AgentSectionView, ...]
...
class AgentReportExperience:
    """Projection of an already-governed AGENT canonical report into Agent UI data."""

    def build(self, payload: dict[str, Any]) -> AgentReportView:
        metadata = payload.get("metadata") or {}
        if metadata.get("report_variant") != "AGENT":
            raise ValueError("AgentReportExperience requires an AGENT report payload")
        ...
        required_text = {
            ...
            "dependency_manifest_hash": lineage.get("dependency_manifest_hash"),
        }
        missing = sorted(key for key, value in required_text.items() if not isinstance(value, str) or not value.strip())
        if missing:
            raise ValueError(f"missing required Agent report fields: {missing}")

        findings_by_id: dict[str, dict[str, Any]] = {}
        for finding in payload.get("findings") or []:
            fid = str(finding.get("finding_id") or "")
            if not fid or fid in findings_by_id:
                raise ValueError("Agent report finding IDs must be nonempty and unique")
        ...
        for card in payload.get("cards") or []:
            ...
            if not cid or cid in cards_by_id:
                raise ValueError("Agent report card IDs must be nonempty and unique")
            cards_by_id[cid] = card

        section_views: list[AgentSectionView] = []
        ...
        for section in payload.get("sections") or []:
            ...
            if not sid or sid in seen_sections:
                raise ValueError("Agent report section IDs must be nonempty and unique")
            if order <= last_order:
                raise ValueError("Agent report sections must preserve canonical ascending order")
            seen_sections.add(sid)
            last_order = order

            card_views: list[AgentCardView] = []
            for card_id in section.get("card_ids") or []:
                if card_id not in cards_by_id:
                    raise ValueError(f"Agent report section references unknown card: {card_id}")
                card = cards_by_id[card_id]
                if card.get("section_id") != sid:
                    raise ValueError(f"Agent report card belongs to a different section: {card_id}")

                finding_views: list[AgentFindingView] = []
                for finding_id in card.get("finding_ids") or []:
                    if finding_id not in findings_by_id:
                        raise ValueError(f"Agent report card references unknown finding: {finding_id}")
                    finding = findings_by_id[finding_id]
                    if finding.get("section_id") != sid:
                        raise ValueError(f"Agent report finding belongs to a different section: {finding_id}")
                    finding_views.append(
                        AgentFindingView(
                            ...
                            source_snapshot_id=str(finding["source_snapshot_id"]),
                            semantic_fingerprint=str(finding["semantic_fingerprint"]),
                        )
                    )
        ...
        return AgentReportView(
            ...
            snapshot_id=str(lineage["snapshot_id"]),
            dependency_manifest_hash=str(lineage["dependency_manifest_hash"]),
        )
