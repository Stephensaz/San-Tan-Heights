...
@dataclass(frozen=True)
class SellerCardView:
    card_id: str
    card_type: str
    title: str
    body: str | None
    status_label: str | None
    findings: tuple[SellerFindingView, ...]


@dataclass(frozen=True)
class SellerSectionView:
    section_id: str
    title: str
    summary: str | None
    cards: tuple[SellerCardView, ...]


@dataclass(frozen=True)
class SellerReportView:
    property_id: str
    address: str
    community: str
    phase: str | None
    ...
    summary: tuple[str, ...]
    sections: tuple[SellerSectionView, ...]
...
class SellerReportExperience:
    """Projection of an already-governed SELLER canonical report into Seller UI data."""

    def build(self, payload: dict[str, Any]) -> SellerReportView:
        metadata = payload.get("metadata") or {}
        if metadata.get("report_variant") != "SELLER":
            raise ValueError("SellerReportExperience requires a SELLER report payload")
        ...
        if missing:
            raise ValueError(f"missing required Seller report fields: {missing}")

        findings_by_id: dict[str, dict[str, Any]] = {}
        for finding in payload.get("findings") or []:
            fid = str(finding.get("finding_id") or "")
            if not fid or fid in findings_by_id:
                raise ValueError("Seller report finding IDs must be nonempty and unique")
        ...
        for card in payload.get("cards") or []:
            ...
            if not cid or cid in cards_by_id:
                raise ValueError("Seller report card IDs must be nonempty and unique")
            cards_by_id[cid] = card

        section_views: list[SellerSectionView] = []
        ...
        for section in payload.get("sections") or []:
            ...
            if not sid or sid in seen_sections:
                raise ValueError("Seller report section IDs must be nonempty and unique")
            if order <= last_order:
                raise ValueError("Seller report sections must preserve canonical ascending order")
            seen_sections.add(sid)
            last_order = order

            card_views: list[SellerCardView] = []
            for card_id in section.get("card_ids") or []:
                if card_id not in cards_by_id:
                    raise ValueError(f"Seller report section references unknown card: {card_id}")
                card = cards_by_id[card_id]
                if card.get("section_id") != sid:
                    raise ValueError(f"Seller report card belongs to a different section: {card_id}")

                finding_views: list[SellerFindingView] = []
                for finding_id in card.get("finding_ids") or []:
                    if finding_id not in findings_by_id:
                        raise ValueError(f"Seller report card references unknown finding: {finding_id}")
                    finding = findings_by_id[finding_id]
                    if finding.get("section_id") != sid:
                        raise ValueError(f"Seller report finding belongs to a different section: {finding_id}")
