...
class PublicPdfCard:
    ...
    body: str | None
    status_label: str | None
    findings: tuple[PublicPdfFinding, ...]


@dataclass(frozen=True)
class PublicPdfSection:
    section_id: str
    title: str
    summary: str | None
    cards: tuple[PublicPdfCard, ...]
...
class PublicPdfDocument:
    ...
    summary: tuple[str, ...]
    sections: tuple[PublicPdfSection, ...]
...
class PublicPdfComposer:
    """Compose a PUBLIC-only PDF document model from the governed Public view."""

    def __init__(self, foundation: PdfTemplateFoundation) -> None:
        self.foundation = foundation

    def compose(self, report: PublicReportView) -> PublicPdfDocument:
        if not isinstance(report, PublicReportView):
            raise ValueError("PublicPdfComposer requires a PublicReportView")
