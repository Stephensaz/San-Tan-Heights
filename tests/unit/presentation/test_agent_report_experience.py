...
from src.presentation.agent import AgentReportExperience


def sample_payload():
    return {
        "metadata": {
            "report_variant": "AGENT",
        ...
        "findings": [
            ...
            },
            {
                ...
                "section_id": "evidence",
                "status_label": "Verified",
                "confidence_label": "Detailed",
                "limitation": "Based on governed source geometry.",
                "source_snapshot_id": "22222222-2222-2222-2222-222222222222",
                "semantic_fingerprint": "b" * 64,
            },
        ],
        "disclaimers": ["For professional use; governed evidence only."],
        "lineage": {
...
def test_agent_report_experience_preserves_governed_content_and_order():
    view = AgentReportExperience().build(sample_payload())
    assert view.property_id == "11111111-1111-1111-1111-111111111111"
    assert [section.section_id for section in view.sections] == ["summary", "evidence"]
    assert view.sections[0].cards[0].findings[0].display_text == "B-3"
    assert view.sections[1].cards[0].findings[0].confidence_label == "Detailed"
    assert view.sections[1].cards[0].findings[0].limitation == "Based on governed source geometry."
    assert len(view.fingerprint) == 64


def test_agent_report_experience_fingerprint_is_deterministic():
    builder = AgentReportExperience()
    assert builder.build(sample_payload()).fingerprint == builder.build(copy.deepcopy(sample_payload())).fingerprint


def test_agent_report_experience_rejects_non_agent_payload():
    payload = sample_payload()
    payload["metadata"]["report_variant"] = "SELLER"
    with pytest.raises(ValueError, match="requires an AGENT"):
        AgentReportExperience().build(payload)


def test_agent_report_experience_rejects_broken_card_reference():
    payload = sample_payload()
...
def test_agent_report_experience_rejects_broken_finding_reference():
    payload = sample_payload()
...
def test_agent_report_experience_rejects_manual_section_reordering():
    payload = sample_payload()
    payload["sections"][0]["order"] = 2
    payload["sections"][1]["order"] = 1
