...
from src.presentation.public import PublicReportExperience


def sample_payload():
    return {
        "metadata": {
            "report_variant": "PUBLIC",
        ...
        "summary": ["Public governed summary text."],
        "sections": [
            {"section_id": "summary", "title": "Summary", "order": 1, "card_ids": ["card-1"], "summary": None},
            {"section_id": "evidence", "title": "Evidence", "order": 2, "card_ids": ["card-2"], "summary": "Public evidence context"},
        ],
        "cards": [
            {"card_id": "card-1", "card_type": "SUMMARY_CARD", "section_id": "summary", "title": "Summary", "body": None, "finding_ids": ["finding-1"], "status_label": None},
            {"card_id": "card-2", "card_type": "EVIDENCE_CARD", "section_id": "evidence", "title": "Evidence", "body": "Public source-backed context", "finding_ids": ["finding-2"], "status_label": "Verified"},
        ],
        "findings": [
            {
...
def test_public_experience_preserves_public_content_and_order():
    view = PublicReportExperience().build(sample_payload())
    ...
    assert view.sections[1].cards[0].findings[0].label == "Road Relationship"
    assert len(view.fingerprint) == 64


def test_public_view_omits_internal_lineage_fields():
    view = PublicReportExperience().build(sample_payload())
    ...
    assert not hasattr(view, "snapshot_id")
    assert not hasattr(view, "dependency_manifest_hash")


def test_public_experience_fingerprint_is_deterministic():
    builder = PublicReportExperience()
    assert builder.build(sample_payload()).fingerprint == builder.build(copy.deepcopy(sample_payload())).fingerprint


def test_public_experience_rejects_non_public_payload():
    payload = sample_payload()
    payload["metadata"]["report_variant"] = "SELLER"
    with pytest.raises(ValueError, match="requires a PUBLIC"):
        PublicReportExperience().build(payload)


def test_public_experience_rejects_broken_card_reference():
    payload = sample_payload()
...
def test_public_experience_rejects_broken_finding_reference():
    payload = sample_payload()
